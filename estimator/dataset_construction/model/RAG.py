...
class BM25OkapiRetriever:
    def __init__(self, documents, top_n=5, ):
...
class Generator:
    ...
    def generate_gpt_response(
        self, 
        query: str, 
        retrieval_passages: str
    ) -> Optional[str]:
        assert self.use_hf == False
        assert self.use_chat_template == True
        formatted_chat_template = self.make_rag_prompt(query, retrieval_passages)
    ...
    def generate_hf_response(
        ...
        retrieval_passages: Union[str, List[str]]
    ) -> Union[str, List[str]]:
        """
        ...
        if isinstance(query, str): # A single query
            # Prepare a formatted prompt
            formatted_prompt = self.make_rag_prompt(query, retrieval_passages)
            ...
            batch_retrieval_passages = retrieval_passages
                
            # Prepare formatted prompts
            formatted_prompts = self.make_rag_prompt_batch(queries, batch_retrieval_passages)
    ...
    def generate(
        ...
        if self.use_hf:
            return self.generate_hf_response(query, retrieval_passages)
        else:
            return self.generate_gpt_response(query, retrieval_passages)
